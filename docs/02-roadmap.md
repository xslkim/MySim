...
> 双仿真器:0.9.15(UE4,基线)+ 0.10(UE5,实现),server 均 Windows 原生,Python 均在 WSL2。
> 每个里程碑有明确完成判据,适合用 goal 模式逐段推进。

## M0 环境验证(一切的前提)

- Windows 原生安装两个 server:CARLA 0.10.0 zip(~130GB)+ 0.9.15 包(~30GB,含 Town10)。
- WSL2 建两个客户端环境:`carla-ue5-api==0.10.0`(py3.11)/ `carla==0.9.15`(py3.8),分别连通 localhost:2000(UE5)/ 2010(UE4,端口错开)。
- 验证:两侧各 spawn 车辆 + RGB 相机取帧 + autopilot 5 分钟不崩;记录两侧帧率。
- 基建:`.wslconfig` 内存 ≥40GB;防火墙放行;路径/端口登记 AGENTS.md。
...
- 完成判据:微调后 UE5 闭环 DS/SR 相比 zero-shot 的提升幅度。
