...
- **T2.1(2026-08-30)**:CARLA 0.10 无 `vehicle.lincoln.mkz_2020`,英雄车用 `vehicle.lincoln.mkz`;UE4 侧测试用 `vehicle.audi.tt`。UE4/UE5 同机并行时 TM 端口必须错开(UE5 8000 / UE4 8010),撞 8000 会串台。
- **T2.4(2026-08-30)**:长任务脚本内**不要**用 subprocess 调 `t11_ensure_server.py`(conda run + carla-ue5-api 偶发 segfault,第 15 条 BLOCKED 白停)——直接 import `tools/server_watchdog.py` 调 `server_alive/restart`;跑 ue5harness 需 `PYTHONPATH=/home/xsl/MySim/tools`(agents 模块已 vendor 到 tools/agents,mysim-ue5 已装 networkx)。官方 leaderboard 的 npc_agent 在同步模式下 TickRuntime 频发(6 条仅 1 条 Completed),不可作 trivial agent 对照基准;T2.6 交叉验证改用单元测试 + 系数集一致性验收。
